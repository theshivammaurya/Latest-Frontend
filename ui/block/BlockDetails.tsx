...
const BlockDetails = ({ query }: Props) => {
  ...
  const { data: blockData, isPlaceholderData: isPlaceholderDataBlockReward } =
    useBlock({
      blockNumber: BigInt(data?.difficulty ?? 0),
    });
  ...
  return (
    ...
    >
      ...
      <>
        <DetailsInfoItem.Label
          hint="Validator rewards are set at 20% of the total block reward, ensuring fair compensation for securing the network"
          isLoading={isPlaceholderDataBlockReward}
        >
          Validator Reward
        </DetailsInfoItem.Label>
        <DetailsInfoItem.Value>
          {isPlaceholderDataBlockReward ? (
            <Skeleton
              isLoaded={!isPlaceholderDataBlockReward}
              h="20px"
              maxW="380px"
              w="100%"
            />
            ...
            <>
              <Text>
                {BigNumber(blockData?.ValidatorReward).dividedBy(WEI).toFixed()}{" "}
                {currencyUnits.ether}{" "}
              </Text>
            </>
          )}
        ...
        <DetailsInfoItem.Label
          hint="Burn subsidy, constitutes 50% of the block reward, reducing token supply to enhance long-term value."
          isLoading={isPlaceholderDataBlockReward}
        >
          BurnSubsidy Layer
        </DetailsInfoItem.Label>
        <DetailsInfoItem.Value>
          {isPlaceholderDataBlockReward ? (
            ...
            <>
              <Text>
                {BigNumber(blockData?.BurnSubsidyReward)
                  .dividedBy(WEI)
                  .toFixed()}{" "}
                {currencyUnits.ether}{" "}
              </Text>
        ...
        <DetailsInfoItem.Label
          hint="Ecosystem incentive account for 30% of the block, fostering growth and developement within the network"
          isLoading={isPlaceholderDataBlockReward}
        >
          EcoSystem Incentive
        </DetailsInfoItem.Label>
        <DetailsInfoItem.Value>
          {isPlaceholderDataBlockReward ? (
            ...
            <>
              <Text>
                {BigNumber(blockData?.EcoSystemIncentive)
                  .dividedBy(WEI)
                  .toFixed()}{" "}
                {currencyUnits.ether}{" "}
              </Text>
