...
const AuthModalScreenEmail = ({ onSubmit, isAuth, mixpanelConfig }: Props) => {
  ...
  const onFormSubmit: SubmitHandler<EmailFormFields> = React.useCallback(
    async (formData) => {
      try {
        ...
        if (isAuth) {
          mixpanelConfig?.account_link_info.source !== "Profile" &&
            mixpanel.logEvent(mixpanel.EventTypes.ACCOUNT_LINK_INFO, {
              Source:
                mixpanelConfig?.account_link_info.source ?? "Profile dropdown",
              Status: "OTP sent",
              Type: "Email",
            });
        } else {
        ...
        toast({
          ...
          title: "Error",
          description:
            getErrorObjPayload<{ message: string }>(error)?.message ||
    ...
    [
      ...
      isAuth,
      onSubmit,
      mixpanelConfig?.account_link_info.source,
      toast,
    ]
  );
