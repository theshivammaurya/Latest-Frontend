...
type Props = CommonProps &
  (AddressTokenTxProps | BlockProps | AddressTokenProps);
...
const AddressLink = (props: Props) => {
  ...
  if (type === "transaction") {
    ...
    url = route({
      ...
      query: { ...props.query, height_or_hash: props.blockHeight },
    });
  ...
  const content = (() => {
    if (alias) {
      ...
      if (type === "token") {
        return (
          <TruncatedTextTooltip label={alias}>{text}</TruncatedTextTooltip>
        );
      }
      return (
  ...
  if (isLoading) {
    return (
      <Skeleton className={className} overflow="hidden" whiteSpace="nowrap">
        {content}
      </Skeleton>
    );
  }
  ...
  if (isDisabled) {
    return (
      <chakra.span className={className} overflow="hidden" whiteSpace="nowrap">
        {content}
      </chakra.span>
    );
  }
